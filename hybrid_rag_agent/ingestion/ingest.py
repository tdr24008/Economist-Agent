"""
Main ingestion script for processing documents into Weaviate and Neo4j knowledge graph.
"""
...
class DocumentIngestionPipeline:
    """Pipeline for ingesting documents into Weaviate and Neo4j knowledge graph."""
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Save to Weaviate
        document_id = await self._save_to_weaviate(
            document_title,
            document_source,
            document_content,
            embedded_chunks,
        ...
        logger.info(f"Saved document to Weaviate with ID: {document_id}")
    ...
    async def _save_to_weaviate(
        ...
        """Save document and chunks to Weaviate."""
        import uuid
        ...
        for chunk in chunks:
            ...
            weaviate_documents.append(weaviate_doc)

        # Batch insert into Weaviate
        if weaviate_documents:
            await self.weaviate_client.batch_insert(weaviate_documents)
            logger.info(f"Inserted {len(weaviate_documents)} chunks into Weaviate")
    ...
    async def _clean_databases(self):
        ...
        # Clean Weaviate
        if self.weaviate_client:
            await self.weaviate_client.delete_all()
            logger.info("Cleaned Weaviate database")
